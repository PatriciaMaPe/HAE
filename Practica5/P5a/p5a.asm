...
;p5a.c,18 :: 		void interrupt() // se ha pulsado una tecla
;p5a.c,20 :: 		key = tecla(); // en la variable key se guarda el valor ASCII de la tecla pulsada
	CALL        _tecla+0, 0
	MOVF        R0, 0 
	MOVWF       _key+0 
;p5a.c,22 :: 		Lcd_Chr_CP (key);
	MOVF        R0, 0 
	MOVWF       FARG_Lcd_Chr_CP_out_char+0 
	CALL        _Lcd_Chr_CP+0, 0
;p5a.c,24 :: 		x = PORTB; // para poder borrar el bit RBIF (define x global)
	MOVF        PORTB+0, 0 
	MOVWF       _x+0 
;p5a.c,25 :: 		INTCON.RBIF=0; // se borra el bit RBIF después de llamar a la función tecla()
	BCF         INTCON+0, 0 
;p5a.c,26 :: 		}
L_end_interrupt:
L__interrupt15:
...
;p5a.c,28 :: 		void main()
;p5a.c,31 :: 		ADCON1 = 0x07; //configuración de los canales analógicos (AN) como digitales (PIC18F452)
	MOVLW       7
	MOVWF       ADCON1+0 
;p5a.c,32 :: 		TRISD = 0; // salida digital
	CLRF        TRISD+0 
...
;p5a.c,37 :: 		Lcd_Init();
	CALL        _Lcd_Init+0, 0
;p5a.c,40 :: 		INTCON2.RBPU = 0; // se habilitan las resistencias de pullup del puerto B
	BCF         INTCON2+0, 7 
;p5a.c,41 :: 		x = PORTB; //para poder borrar el RBIF
	MOVF        PORTB+0, 0 
	MOVWF       _x+0 
;p5a.c,42 :: 		INTCON.RBIF=0;
	BCF         INTCON+0, 0 
;p5a.c,43 :: 		INTCON.RBIE=1;
	BSF         INTCON+0, 3 
;p5a.c,44 :: 		INTCON.GIE = 1; // se habilitan las interrupciones en general
	BSF         INTCON+0, 7 
;p5a.c,47 :: 		while(1){
L_main11:
;p5a.c,49 :: 		}
